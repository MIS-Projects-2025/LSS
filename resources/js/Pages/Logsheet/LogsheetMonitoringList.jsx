...
export default function LogsheetMonitoringList({
    tableData,
    tableFilters,
    emp_data,
    stampNo,
...
const addToCart = async () => {
    ...
    router.put(route("monitoring.logsheets.update", activeRow.id), values, {
        onSuccess: () => {
            message.success("Logsheet item updated successfully!");
            setManageOpen(false);
            router.reload(); // SPA-friendly refresh
        },
        onError: (errors) => {
            console.error(errors); // log full errors for debugging
            message.error("Failed to update Logsheet item.");
        },
    ...
    const tableDataRows = (tableData?.data || []).map((row, index) => ({
        ...
        // 🔸 inspection_result badge
 inspection_result: (
  <span
    className={`px-2 py-1 text-xs font-semibold border rounded-md ${
      row.inspection_result?.trim().toUpperCase() === "PASSED"
        ? " text-green-800 border-green-500 hover:bg-green-700 hover:text-white"
        : " text-red-800 border-red-500 hover:bg-red-700 hover:text-white"
    }`}
  >
    ...
    return (
        <AuthenticatedLayout>
            ...
            <div className="flex justify-between mb-4">
                <h1 className="text-2xl font-bold text-yellow-500"><i className="fa-solid fa-file-circle-check"></i> Inspection Monitoring Logsheet List</h1>
            </div>
            ...
            <DataTable
                columns={[
                    ...
                    { key: "employee_name", label: "Inspected By" },
                ...
                data={tableDataRows}
                meta={tableData}
                routeName={route("inspection.logsheet.list")}
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="p-4 rounded-lg bg-white shadow-sm border border-gray-200">
                <Form.Item name="remarks" label="Remarks">
                    <TextArea className="rounded-md bg-white"  rows={4} />
                </Form.Item>
            </div>
        </div>
